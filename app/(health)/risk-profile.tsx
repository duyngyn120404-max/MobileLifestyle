...
import type { ClinicalFact } from "@/src/repositories/home-health.repository";
...
export default function RiskProfileScreen() {
  ...
  const [initialRiskFactors, setInitialRiskFactors] = useState<string[]>([]);
  const [initialHmodItems, setInitialHmodItems] = useState<string[]>([]);
  const [initialCardiovascularDiseases, setInitialCardiovascularDiseases] =
    useState<string[]>([]);
  ...
  const hydrateFacts = (facts: ClinicalFact[]) => {
    const nextRisk = uniqueStrings(
      facts
        .filter((item) => item.fact_group === "risk_factors")
        .map((item) => item.fact_key),
    );

    const nextHmod = uniqueStrings(
      facts
        .filter((item) => item.fact_group === "hmod")
        .map((item) => item.fact_key),
    );

    const nextCvd = uniqueStrings(
      facts
        .filter((item) => item.fact_group === "cardiovascular_disease")
        .map((item) => item.fact_key),
    );

    setRiskFactors(nextRisk);
    setHmodItems(nextHmod);
    setCardiovascularDiseases(nextCvd);

    setInitialRiskFactors(nextRisk);
    setInitialHmodItems(nextHmod);
    setInitialCardiovascularDiseases(nextCvd);
  };
  ...
  const loadProfile = async () => {
    ...
    try {
      setLoading(true);
      const facts = await homeHealthService.getRiskProfile(user.id);
      hydrateFacts(Array.isArray(facts) ? facts : []);
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Không thể tải hồ sơ nguy cơ";
  ...
  const hasChanges = useMemo(() => {
    return (
      JSON.stringify(uniqueStrings(riskFactors).sort()) !==
        JSON.stringify(uniqueStrings(initialRiskFactors).sort()) ||
      JSON.stringify(uniqueStrings(hmodItems).sort()) !==
        JSON.stringify(uniqueStrings(initialHmodItems).sort()) ||
      JSON.stringify(uniqueStrings(cardiovascularDiseases).sort()) !==
        JSON.stringify(uniqueStrings(initialCardiovascularDiseases).sort())
    );
  }, [
    riskFactors,
    hmodItems,
  ...
  const addCustomItem = (
    ...
    setter((prev) => uniqueStrings([...prev, normalized]));
    reset();
  };
  ...
  const handleCancelEdit = () => {
    ...
    setHmodItems(initialHmodItems);
    setCardiovascularDiseases(initialCardiovascularDiseases);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await homeHealthService.saveRiskProfile({
        userId: user.id,
        riskFactors: uniqueStrings(riskFactors),
        hmodItems: uniqueStrings(hmodItems),
        cardiovascularDiseases: uniqueStrings(cardiovascularDiseases),
        source: "user",
      });

      setInitialRiskFactors(uniqueStrings(riskFactors));
      setInitialHmodItems(uniqueStrings(hmodItems));
      setInitialCardiovascularDiseases(uniqueStrings(cardiovascularDiseases));

      setIsEditing(false);
      showMessage("Đã lưu hồ sơ nguy cơ");
    } catch (error) {
      const message =
        ...
        <Card style={styles.stageCard}>
          <Card.Content>
            <MultiSelectSection
              ...
              options={riskOptions}
              values={riskFactors}
              onToggle={(key) => toggleItem(key, setRiskFactors)}
            ...
            {isEditing ? (
              <CustomAddRow
                ...
                onAdd={() =>
                  addCustomItem(customRisk, setRiskFactors, () =>
                    setCustomRisk(""),
                  )
                }
        ...
        <Card style={styles.stageCard}>
          <Card.Content>
            <MultiSelectSection
              ...
              options={hmodOptions}
              values={hmodItems}
              onToggle={(key) => toggleItem(key, setHmodItems)}
              disabled={!isEditing || loading}
            />
            ...
            {isEditing ? (
              <CustomAddRow
                ...
                onAdd={() =>
                  addCustomItem(customHmod, setHmodItems, () =>
                    setCustomHmod(""),
                  )
                }
        ...
        <Card style={styles.stageCard}>
          <Card.Content>
            <MultiSelectSection
              ...
              onToggle={(key) => toggleItem(key, setCardiovascularDiseases)}
              disabled={!isEditing || loading}
            />
            ...
            {isEditing ? (
              <CustomAddRow
                ...
                onAdd={() =>
                  addCustomItem(customCvd, setCardiovascularDiseases, () =>
                    setCustomCvd(""),
                  )
                }
